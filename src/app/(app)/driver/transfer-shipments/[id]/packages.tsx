import { getTransferShipmentPackages } from "@/api/transfer-shipment"
...
export default function ViewDeliveryPackagesPage() {
  ...
  const { status, data, error } = useQuery({
    ...
    queryFn: () => getTransferShipmentPackages(Number(id)),
  })
            ...
            >
              {
                data.packages.filter(
                  (_package: any) => _package.status === "TRANSFERRED",
                ).length
              }
          ...
          >
            {data.packages.map((_package: any) => (
              <View
                ...
                style={{
                  backgroundColor:
                    _package.status === "TRANSFERRED" ? "#22c55e" : "#f97316",
                  paddingVertical: 8,
                  paddingHorizontal: 12,
                  borderRadius: 8,
                }}
